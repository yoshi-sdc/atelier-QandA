...
module.exports = {
  getAnswers: async function getAnswers(q_id, page, count, callback) {
    try {
      ...
      const allAnswers = `SELECT a_id AS answer_id, body, TO_TIMESTAMP(date / 1000)::date, answerer_name, helpfulness,
  ...
  addAnswer: async function addAnswer(question_id, body, name, email, photos) {
    try {
      const addA = `INSERT INTO answers
        ...
        .then((res) => {
          ...
          if (photos.length > 0) {
            for (let i = 0; i < photos.length; i += 1) {
              const url = photos[i];
              const addPhotos = `INSERT INTO photos
                                   (answer_id, url)
                                 VALUES (${answerId}, '${url}')`;
